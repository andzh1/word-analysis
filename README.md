...
## Process and implementation
 ...
 For each word in given list of words we calculate it's sum of matches (as sum of matches in our dataset of all substrings of `SEQUENCE_SIZE` of our word); then for each  *[shifted](#Definitions)* word we calculate it's sum of matches and check, if sum of matches of our words is biggest of all those sums. If it's bigger, we say that our algorithm works correctly on this word, in other case it is incorrect.
